...
def print_subnet_info(addr, total_ips, subnets):
    ...
    for i in range(subnets):
        ...
        print(f"Range of usable IPs: {subnet_addr[0]}.{subnet_addr[1]}.{subnet_addr[2]}.{subnet_addr[3] + 1} to {broadcast_addr[0]}.{broadcast_addr[1]}.{broadcast_addr[2]}.{broadcast_addr[3] - 1}")

def calculate_subnet_mask(net_class, subnets):
    borrowed_bits = math.ceil(math.log2(subnets))  # Number of bits to borrow

    if net_class == 'A':
        total_host_bits = 24  # Class A has 24 host bits
    elif net_class == 'B':
        total_host_bits = 16  # Class B has 16 host bits
    elif net_class == 'C':
        total_host_bits = 8  # Class C has 8 host bits
    else:
        return "N/A"

    remaining_host_bits = total_host_bits - borrowed_bits

    mask_bits = 32 - remaining_host_bits  # Total number of network bits
    ...
    for i in range(4):
        if mask_bits >= 8:
            ...
            subnet_mask += str(256 - 2 ** (8 - mask_bits))
            mask_bits = 0
        else:
            subnet_mask += "0"
